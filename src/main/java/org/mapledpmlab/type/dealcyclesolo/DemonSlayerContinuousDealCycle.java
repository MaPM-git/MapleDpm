...
import org.mapledpmlab.type.JobContinuous.DemonSlayerNormalContinuous;
...
public class DemonSlayerContinuousDealCycle extends DealCycle {
    ...
    public DemonSlayerContinuousDealCycle(Job job) {
        super(job, null);

        if (getJob() instanceof DemonSlayerNormalContinuous) {
            getJob().setName("데몬슬레이어(컨티, 극포실, 환산 84502)");
        }
        ...
        while (getStart().before(getEnd())) {
            ...
            if (
                    cooldownCheck(otherWorldGoddessBlessing)
                            && getStart().after(new Timestamp(soulContract.getActivateTime().getTime() - 1500))
                            && demonForce >= 100
            ) {
                isNuke = true;
