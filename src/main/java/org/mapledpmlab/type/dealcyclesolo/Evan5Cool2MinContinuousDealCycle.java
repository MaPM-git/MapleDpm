...
public class Evan5Cool2MinContinuousDealCycle extends DealCycle {
    ...
    public Evan5Cool2MinContinuousDealCycle(Job job) {
        super(job, new DragonSpark());

        this.getJob().setName("에반(컨티, 5초, 2분)");
        ...
        while (getStart().before(getEnd())) {
            if (cooldownCheck(crestOfTheSolar)) {
                ...
                addSkillEvent(freudBlessing6);
                    ...
                    cooldownCheck(zodiacRayDelay)
                            && getStart().after(new Timestamp(elementalBlast.getActivateTime().getTime() - 2500))
                ...
                addSkillEvent(spiralOfMana);
                    ...
                    cooldownCheck(summonOnyxDragon)
                            && getStart().before(new Timestamp(zodiacRayDelay.getActivateTime().getTime() - 40000))
            ) {
                addSkillEvent(summonOnyxDragon);
            } else if (
                ...
                addSkillEvent(soulContract);
                    ...
                    cooldownCheck(dragonBreak)
                            ...
                            && getStart().before(new Timestamp(zodiacRayDelay.getActivateTime().getTime() - 10000))
            ) {
                addSkillEvent(dragonBreak);
                boolean som = false;
                for (int i = 0; i < 3; i++) {
