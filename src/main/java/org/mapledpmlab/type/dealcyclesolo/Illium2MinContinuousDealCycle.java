...
public class Illium2MinContinuousDealCycle extends DealCycle {
    ...
    Timestamp subNukeTime = new Timestamp(1000000000);
    ...
    public Illium2MinContinuousDealCycle(Job job) {
        super(job, new CurseMark());

        this.setAttackSkillList(attackSkillList);
        this.setBuffSkillList(buffSkillList);

        this.getJob().setName("일리움(컨티, 2분)");
        ...
        while (getStart().before(getEnd())) {
            if (
                ...
                if (cooldownCheck(grandisGoddessBlessingLef)) {
                    ...
                    subNukeTime = new Timestamp(1000000000);
                } else {
                ...
                while (cooldownCheck(fastCharge)) {
                    ...
                    cooldownCheck(grandisGoddessBlessingLef)
                    && getStart().before(new Timestamp(unlimitedCrystal1.getActivateTime().getTime() - 30000))
            ) {
                ...
                subNukeTime = new Timestamp(getStart().getTime());
            } else if (
                    getStart().after(crystalSkillGloryWingEndTime)
                            && crystalCharge >= 150
                            && cooldownCheck(crystalSkillDeus)
                            && getStart().before(new Timestamp(gramholder.getActivateTime().getTime() - 50000))
                            && getStart().after(subNukeTime)
            ) {
                addSkillEvent(crystalSkillDeus);
                addSkillEvent(crystalSkillMortalSwing);
                addSkillEvent(crystalSkillGloryWing);
