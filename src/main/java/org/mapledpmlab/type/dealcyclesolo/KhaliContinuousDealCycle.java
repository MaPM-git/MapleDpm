...
public class KhaliContinuousDealCycle extends DealCycle {
    ...
    public KhaliContinuousDealCycle(Job job) {
        super(job, null);
        ...
        resonateUltimatum.setActivateTime(new Timestamp(-55555555));
    }
    ...
    public void setSoloDealCycle() {
        while (getStart().before(getEnd())) {
            if (
                    getStart().after(new Timestamp(resonateUltimatum.getActivateTime().getTime() + 500))
            ...
            if (((AttackSkill) skill).getInterval() != 0) {
                ...
                if (((AttackSkill) skill).getLimitAttackCount() == 0) {
                    ...
                    for (long i = ((AttackSkill) skill).getInterval(); i <= ((AttackSkill) skill).getDotDuration() && attackCount < ((AttackSkill) skill).getLimitAttackCount(); i += ((AttackSkill) skill).getInterval()) {
                        ...
                        if (
                                skill instanceof VoidBurstCombo
                                || skill instanceof HexSandStormKeydown
                                || skill instanceof HexChakramFury
                        ) {
                            Timestamp temp = new Timestamp(getStart().getTime());
                            getStart().setTime(getStart().getTime() + i);
    ...
    public void multiAttackProcess(Skill skill) {
        ...
        for (Long info : ((AttackSkill) skill).getMultiAttackInfo()) {
            ...
            if (
                    skill instanceof HexSandStormBomb
                    || skill instanceof VoidBlitz
                    || skill instanceof VoidBurstSlash
            ) {
                Timestamp temp = new Timestamp(getStart().getTime());
                getStart().setTime(getStart().getTime() + sum);
                if (cooldownCheck(continuousRing)) {
