...
public class CannonShooter2DealCycle extends DealCycle {

    private final List<AttackSkill> attackSkillList = new ArrayList<>(){
        {
            add(new BarrelRouletteSkull());
            add(new BigFuseGiganticCannonball());
            add(new BigFuseGiganticCannonball1Stack());
    ...
    BigFuseGiganticCannonball1Stack bigFuseGiganticCannonball1Stack = new BigFuseGiganticCannonball1Stack();
    ...
    public CannonShooter2DealCycle(Job job) {
        super(job, new FinalAttackCannonShooter());

        getJob().setName("캐논슈터(리웨, 2분)");
    ...
    public void setSoloDealCycle() {
        while (getStart().before(getEnd())) {
            ...
            if (cooldownCheck(mapleWorldGoddessBlessing)) {
                ...
                while (!cooldownCheck(pirateFlag)) {
                    addSkillEvent(cannonBuster);
                    if (cooldownCheck(miniCanonBall)) {
                        addSkillEvent(miniCanonBall);
                    }
                ...
                if (cooldownCheck(superCannonExplosion1)) {
                    ...
                    cooldownCheck(bigFuseGiganticCannonball1Stack)
                    && getStart().before(new Timestamp(specialMonkeyEscort.getActivateTime().getTime() - 40000))
            ) {
                addSkillEvent(bigFuseGiganticCannonball1Stack);
            } else if (cooldownCheck(soulContract)) {
                addSkillEvent(pirateFlag);
                addSkillEvent(poolmakerBuff);
                addSkillEvent(overdrive);
                addSkillEvent(soulContract);
                ...
                addSkillEvent(ringSwitching);
                    ...
                    && getStart().before(new Timestamp(overdrive.getActivateTime().getTime() - 10000))
                    && getStart().before(new Timestamp(11 * 60 * 1000))
            ) {
                addSkillEvent(pirateFlag);
            } else if (
    ...
    public void addSkillEvent(Skill skill) {
        ...
        if (getStart().before(skill.getActivateTime())) {
            System.out.println(getStart() + "\t" + skill.getName() + "\t" + getJob().getName() + "\t쿨 : " + skill.getActivateTime());
            return;
        }
        if (skillLog.equals("")) {
        ...
        applyCooldown(skill);
        if (skill instanceof BigFuseGiganticCannonball) {
            applyCooldown(bigFuseGiganticCannonball1Stack);
        } else if (skill instanceof BigFuseGiganticCannonball1Stack) {
            applyCooldown(bigFuseGiganticCannonball);
        }
