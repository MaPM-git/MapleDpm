...
public class MercedesContinuousSylphidiaDealCycle extends DealCycle {
    ...
    public MercedesContinuousSylphidiaDealCycle(Job job) {
        super(job, new FinalAttackMercedes());

        this.getJob().setName("메르세데스(컨티, 실피디아 캔슬)");
        ...
        freudBlessing6.setCooldown(255.0);
    }
        ...
        while (getStart().before(getEnd())) {
            if (
                ...
                isNuke = false;
                    ...
                    cooldownCheck(sylphidia)
                    && !cooldownCheck(heroesOath)
                    && getStart().after(elementalGhostEndTime)
            ) {
                addSkillEvent(sylphidia);
            } else if (
                    cooldownCheck(soulContract)
                ...
                addSkillEvent(leafTornado);
                    ...
                    && cooldownCheck(sylphidiaRush)
            ) {
                addSkillEvent(sylphidiaRush);
                if (cooldownCheck(wrathOfEnlil)) {
                    addSkillEvent(wrathOfEnlil);
                } else if (cooldownCheck(unicornSpike)) {
                    unicornSpike.setCooldown(9.0);
                    addSkillEvent(unicornSpike);
                    unicornSpike.setCooldown(0.0);
                } else if (cooldownCheck(legendarySpear)) {
                    legendarySpear.setCooldown(5.0);
                    addSkillEvent(legendarySpear);
                    legendarySpear.setCooldown(0.0);
                }
